...
package io.github.nfm1337.lwjgl3
 ...
 * - macOS: Spawns a child JVM process with `-XstartOnFirstThread` in the JVM args
 *   (if it was not already). This is required for LWJGL3 to work on macOS.
 * - Linux (NVIDIA GPUs only): Spawns a child JVM process with the
 *   `__GL_THREADED_OPTIMIZATIONS` [Environment Variable][System.getenv] set to `0`
 *   (if it was not already). This is required for LWJGL3 to work on Linux with NVIDIA GPUs.
 *
...
object StartupHelper {
    ...
    fun isLinuxNvidia(): Boolean =
        File("/proc/driver")
            .list { _, path: String -> "NVIDIA" in path.uppercase() }
            .isNullOrEmpty()
            .not()
     ...
     *
     * All [Environment Variables][System.getenv] are copied to the child JVM process
     * (if it is spawned), as specified by [ProcessBuilder.environment]; the same applies
     * for [System Properties][System.getProperties].
     *
     * **Usage:**
     *
     * ```
     ...
     * }
     * ```
     * @param inheritIO whether I/O should be inherited in the child JVM process. Please note
     * that enabling this will block the thread until the child JVM process stops executing.
    ...
    fun startNewJvmIfRequired(inheritIO: Boolean = true): Boolean {
        ...
        return when {
            "mac" in osName -> {
                startNewJvm0(isMac = true, inheritIO)
            }

            "windows" in osName -> {
                prepareWindowsNatives()
                false
            }

            else -> {
                startNewJvm0(isMac = false, inheritIO)
            }
        }
    }

    private fun prepareWindowsNatives() {
    ...
    private const val CHILD_LOOP_ERR_MSG: String =
        "The current JVM process is a spawned child JVM process, but StartupHelper has attempted to " +
            "spawn another child JVM process! This is a broken state, and should not normally happen! " +
     ...
     * for [System Properties][System.getProperties].
     *
     * @param isMac whether the current OS is macOS. If this is `false` then the current OS
     * is assumed to be Linux (and an immediate check for NVIDIA drivers is performed).
     * @param inheritIO whether I/O should be inherited in the child JVM process. Please note
     * that enabling this will block the thread until the child JVM process stops executing.
     * @return whether a child JVM process was spawned or not.
     */
    ...
    fun startNewJvm0(
        ...
        // This state shouldn't usually be reachable, but this prevents an endless chain of child processes.
        ...
        try {
            ...
            System.err.println("There was a problem restarting the JVM: ${exception.message}")
        } catch (exception: InterruptedException) {
            Thread.currentThread().interrupt()
            System.err.println("Interrupted while waiting for the restarted JVM: ${exception.message}")
        }
